...
PARAMS = {
    ...
    "dropout_rate": 0.3,
    "n_heads": 16,
    "output_dim": 512,   # choose from 128, 256, 512, 1024
...
if __name__ == "__main__":
    ...
    summary_rows = {
        "Accuracy":        [m["acc"] * 100],
        "Loss":            [test_loss],
        "Prec_Macro":      [m["prec_macro"]],
        "Rec_Macro":       [m["rec_macro"]],
        "Spec_Macro":      [m["spec_macro"]],
        "F1_Macro":        [m["f1_macro"]],
        "Prec_Micro":      [m["prec_micro"]],
        "Rec_Micro":       [m["rec_micro"]],
        "Spec_Micro":      [m["spec_micro"]],
        "F1_Micro":        [m["f1_micro"]],
        "Prec_Weighted":   [m["prec_weighted"]],
        "Rec_Weighted":    [m["rec_weighted"]],
        "Spec_Weighted":   [m["spec_weighted"]],
    ...
    pd.DataFrame(summary_rows).to_csv(f"batch_size/results_fixed_{PARAMS['batch_size']}.csv", index=False)
    m["df_metrics"].to_csv(f"batch_size/metrics_table_{PARAMS['batch_size']}.csv", index=False)
