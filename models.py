...
class ecgTransForm(nn.Module):
    def __init__(self, configs, hparams):
        ...
        self.encoder_layer = nn.TransformerEncoderLayer(d_model=128, nhead=32, batch_first=True)
    ...
    def forward(self, x_in):
        ...
        x_in = torch.mean(torch.stack([x1, x2, x3],2), 2)
        x_in = self.do(self.mp(self.relu(self.bn(x_in))))
        ...
        x = x1+x2
        x = x.permute(0, 2, 1)  # (batch_size, dim, seq_len)
